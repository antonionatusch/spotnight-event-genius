...
import { STAFF_DEMO_PASSWORD } from '@/lib/demo-access';
import { useStore } from '@/lib/store';
import { Shield, Camera, Search, CheckCircle2, XCircle, AlertCircle, Trash2 } from 'lucide-react';
import { toast } from 'sonner';
import {
  cancelRemoteReservation,
  checkInRemoteReservation,
  parseQrPayload,
...
function CheckIn() {
  return (
    <PasswordGate
      ...
      subtitle="Ingresá la clave para validar QRs y gestionar cancelaciones desde puerta."
...
function CheckInContent() {
  ...
  const validateCode = async (rawCode: string) => {
    ...
    try {
      ...
      if (isSupabaseConfigured) {
        ...
        toast.error('No se pudo validar con Supabase; no se registró el ingreso', {
          id: 'check-in-result',
        });
        return;
      }
      ...
      if (r.ok)
        toast.success(`Ingreso local registrado: ${r.reservation?.code}`, {
          id: 'check-in-result',
        });
      else
        toast.error(r.duplicate ? r.message : 'No se pudo validar con Supabase', {
          id: 'check-in-result',
        });
    } finally {
  ...
  const onStaffCancel = async () => {
    const parsed = parseQrPayload(code);
    const normalizedCode = parsed.code.trim().toUpperCase();
    const target = reservations.find(
      (reservation) =>
        reservation.code.toUpperCase() === normalizedCode &&
        (!parsed.eventId || parsed.eventId === reservation.eventId),
    );

    if (!target) {
      toast.error('Reserva no encontrada para este evento');
      return;
    }

    if (target.status === 'Cancelada') {
      toast.error('La reserva ya está cancelada');
      return;
    }

    if (!window.confirm(`Cancelar reserva ${target.code} de ${target.userName}?`)) return;

    setCancellingReservation(true);
    try {
      if (isSupabaseConfigured) {
        const updated = await cancelRemoteReservation(target.id, 'staff');
        upsertReservation(updated);
      } else {
        cancelReservation(target.id, 'staff');
      }

      toast.success(getReservationCancellationMessage('staff'), {
        description: `Reserva ${target.code}`,
      });
      setCode('');
      setResult(null);
    } catch (error) {
    ...
    <div className="px-4 py-4 space-y-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              {event.date} · {event.startTime} hs · En curso
            </p>
          </div>
        </div>
            ...
            <div className="flex min-h-48 flex-col items-center justify-center gap-3 p-6">
              <div className="relative h-32 w-32">
                <div
                  ...
                  style={{
                    borderImage:
                      'linear-gradient(135deg, var(--color-primary), var(--color-magenta)) 1',
                  }}
                />
      ...
      <div className="space-y-2">
        ...
        <p className="text-[11px] text-muted-foreground">
          Acepta código corto o JSON del QR · Prueba: SN-001 (válida) · SN-002 (usada)
        </p>
        <button
          onClick={onValidate}
          disabled={!code.trim() || validating || cancellingReservation}
        ...
        <button
          ...
          disabled={!code.trim() || validating || cancellingReservation}
          className="flex w-full items-center justify-center gap-2 rounded-2xl border border-destructive/50 bg-destructive/10 py-3 text-sm font-bold text-destructive disabled:opacity-40"
        >
          <Trash2 className="h-4 w-4" />{' '}
          {cancellingReservation ? 'Cancelando...' : 'Cancelar reserva con devolución'}
        </button>
      </div>
      ...
      {result && (
        ...
        >
          ...
          <div className="flex-1">
            ...
            {result.resCode && (
              <p className="text-xs opacity-80 mt-0.5">Reserva {result.resCode}</p>
            )}
            {result.duplicate && (
              <p className="text-[11px] opacity-90 mt-1">
                Esta reserva ya fue utilizada. Verificá la identidad del asistente.
              </p>
            )}
            {result.expired && (
              <p className="text-[11px] opacity-90 mt-1">
                El evento finalizó. No se permiten más ingresos.
              </p>
            )}
          </div>
        </div>
