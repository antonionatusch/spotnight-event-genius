...
function ReservationPage() {
  ...
  useEffect(() => {
    ...
    if (latest.type === 'check_in_duplicate') {
      toast.error('Intento de ingreso duplicado · Tu QR ya fue utilizado', {
        id: 'reservation-audit-alert',
      });
  ...
  const latestCancellation = auditEvents.find((event) => event.type === 'reservation_cancelled');
  ...
  return (
    <div>
      ...
      <main className="px-4 py-4 space-y-4">
        ...
        {cancelled && (
          <div className="flex items-start gap-2 rounded-2xl border border-destructive/50 bg-destructive/15 px-4 py-3 text-destructive">
            <XCircle className="h-5 w-5" />
            <div>
              <p className="text-sm font-bold">Reserva cancelada</p>
              <p className="text-xs opacity-90">
                {getReservationCancellationMessage(latestCancellation?.actorRole)}
              </p>
            </div>
          </div>
        )}
        ...
        {latestDuplicate && (
          <div className="rounded-3xl border border-destructive/60 bg-destructive/15 p-5 text-center text-destructive shadow-[0_0_32px_rgba(239,68,68,0.25)]">
            ...
            <p className="mt-1 text-xs opacity-80">
              {new Date(latestDuplicate.createdAt).toLocaleString()}
            </p>
          </div>
        )}
        {!inactive && (
        ...
        <div className={`rounded-3xl p-[2px] gradient-primary ${inactive ? '' : 'glow-primary'}`}>
          <div
            className={`rounded-3xl bg-card p-5 space-y-4 ${inactive ? 'opacity-60 grayscale' : ''}`}
          >
            ...
            <div className="grid grid-cols-2 gap-3 border-t border-border pt-4 text-xs">
              <InfoCell
                icon={<Hash className="h-3 w-3 text-primary" />}
                label="EVENTO"
                value={r.eventName}
                sub={`ID ${r.eventId}`}
              />
              ...
              <InfoCell
                ...
                label="FECHA"
                value="Sáb 14 Jun 2025"
              />
              <InfoCell
                icon={<Layers className="h-3 w-3 text-magenta" />}
                label="PLANTA"
                value={
                  r.floor === 'planta_alta'
                    ? 'Planta Alta / VIP'
              ...
              />
              <InfoCell
                icon={<Tag className="h-3 w-3 text-gold" />}
                label="ZONA"
                value={r.zone ? `Zona ${r.zone}` : r.ticketType}
              />
              <InfoCell
                icon={<MapPin className="h-3 w-3 text-magenta" />}
                label="MESA / SILLA"
                value={r.venueMapItemLabel ?? 'General'}
                accent="text-magenta"
              />
              <InfoCell
                icon={<Users className="h-3 w-3 text-neon" />}
                label="PERSONAS"
                value={`${r.peopleCount} pers.`}
              />
              <InfoCell
            ...
            >
              <p className="text-sm font-bold">
                ● Estado:{' '}
                {cancelled ? 'Cancelada' : expired ? 'Expirada' : used ? 'Ingresó' : 'Válida'}
              </p>
            </div>
          </div>
        </div>
        ...
        <Dialog open={cancelOpen} onOpenChange={setCancelOpen}>
          <DialogContent className="max-w-[92vw] rounded-2xl border-destructive/40 bg-card sm:max-w-md">
            <DialogHeader>
              ...
              <DialogDescription>
                Esta acción no se puede deshacer. Tu lugar{' '}
                {r.venueMapItemLabel ? <b>{r.venueMapItemLabel}</b> : 'asignado'} quedará disponible
                para otros usuarios.
              </DialogDescription>
            </DialogHeader>
