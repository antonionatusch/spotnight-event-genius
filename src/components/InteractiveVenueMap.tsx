...
type Props = {
  ...
  filterTypes?: VenueMapItem["type"][];
  showLegendFilter?: boolean;
};

type LegendFilter = "available" | "vip" | "occupied" | null;

const W = 460;
const H = 560;

export function InteractiveVenueMap({ floor, selectedId, onSelect, filterTypes, showLegendFilter = true }: Props) {
  ...
  const matchesLegend = (it: VenueMapItem) => {
    if (!legendFilter) return true;
    if (!isReservable(it)) return true; // never dim structural elements
    const status = it.status ?? "available";
    if (legendFilter === "available") return status === "available" && it.zone !== "VIP";
    if (legendFilter === "vip") return it.zone === "VIP";
    if (legendFilter === "occupied") return status === "occupied" || status === "reserved";
    return true;
  };
  ...
  return (
    <div className="space-y-3">
      <div className="rounded-2xl border-2 p-2" style={{ borderColor: frameStroke, background: bg }}>
        <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto">
          ...
          {items.map((it) => {
            ...
            const dim = !matchesLegend(it);
            const opacity = dim ? 0.2 : disabled ? 0.55 : 1;

            let shape: React.ReactNode = null;
            if (it.radius) {
              shape = <circle cx={it.x} cy={it.y} r={it.radius} fill={fill} stroke={stroke} strokeWidth={isSelected ? 3 : 2} />;
            } else if (it.type === "stage") {
              shape = <ellipse cx={it.x} cy={it.y} rx={(it.width ?? 100) / 2} ry={(it.height ?? 30) / 2} fill="transparent" stroke={stroke} strokeWidth={2} />;
            ...
            return (
              <g key={it.id} onClick={handleClick} style={{ cursor }} opacity={opacity}>
                {shape}
                <text
                  x={it.x}
                  y={it.y + 4}
      ...
      {showLegendFilter && (
        <InteractiveLegend value={legendFilter} onChange={setLegendFilter} />
      )}
    </div>
  );
}

function InteractiveLegend({ value, onChange }: { value: LegendFilter; onChange: (v: LegendFilter) => void }) {
  const dot = (color: string) => (
    <span className="inline-block h-2.5 w-2.5 rounded-full" style={{ background: color, boxShadow: `0 0 8px ${color}` }} />
  );
  const chip = (key: LegendFilter, color: string, label: string) => {
    ...
    return (
      <button
        ...
        className={`flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-xs transition ${
          active ? "border-primary bg-primary/15 text-foreground" : "border-border bg-card/40 text-muted-foreground hover:text-foreground"
        }`}
      >
        {dot(color)} {label}
      </button>
  ...
  return (
    <div className="flex flex-wrap items-center justify-center gap-2 text-xs">
      {chip("available", "oklch(0.78 0.18 220)", "Disponible")}
      {chip("vip", "oklch(0.82 0.15 85)", "VIP")}
      {chip("occupied", "oklch(0.65 0.24 25)", "Ocupada")}
      <span className="flex items-center gap-1.5 text-muted-foreground"><span className="inline-block h-2.5 w-2.5 rounded-full" style={{ background: "oklch(0.66 0.27 0)" }} /> Seleccionada</span>
      {value && (
        <button onClick={() => onChange(null)} className="text-[11px] text-magenta underline">Limpiar filtro</button>
      )}
    </div>
  );
}
